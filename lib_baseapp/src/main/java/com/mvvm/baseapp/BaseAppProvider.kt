 ...
 * Tel : 13522647431
 */
@Route(path = Provider.INIT_BASE_APP)
class BaseAppProvider : IApplicationProvider {

    override fun init(context: Context) {
        mContext = context as Application
        Log.d("init","BaseAppProvider init");
    }

    override fun attachBaseContext(context: Application) {
        Log.d("init","BaseAppProvider attachBaseContext");
    }
    override fun onCreate(application: Application, isDebug: Boolean) {
        Debug = isDebug
        Log.d("init","BaseAppProvider onCreate");
    }
